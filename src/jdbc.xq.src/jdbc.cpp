  ...
  namespace jdbc {
    ...
    JdbcModule::getExternalFunction(const zorba::String &localName) {
      ...
      if (lIte == lFunctions.end()) {
        // 1 JDBC
        if (localName == "register-driver") {
          lFunc = new RegisterFunction(this);
        }
        // 2 CONNECTION HANDLING
        else if (localName == "connect") {
    ...
    JdbcModule::throwError(const char *aLocalName, String aErrorMessage) {
      ...
      Item errQName = Zorba::getInstance(0)->getItemFactory()->createQName(
          errNS, errName);
    ...
    JdbcModule::throwJavaException(JNIEnv *env, jthrowable &lException) {
      ...
      jmethodID toStringMethod =
          env->GetMethodID(stringWriterClass, "toString",
                           "()Ljava/lang/String;");
    ...
    String
    JdbcModule::getStringArg(const ExternalFunction::Arguments_t &args,
                             int index) {
      String result;
    ...
    bool
    JdbcModule::getOptionalStringArg(const ExternalFunction::Arguments_t &args,
    ...
    Item
    JdbcModule::getItemArg(const ExternalFunction::Arguments_t &args,
                           int index) {
      Item item;
    ...
    InstanceMap *
    JdbcModule::getCreateInstanceMap(
        const zorba::DynamicContext *aDynamincContext, String mapName) {

      InstanceMap *result;
      DynamicContext *lDctx = const_cast<DynamicContext *>(aDynamincContext);
      if (!(result = dynamic_cast<InstanceMap *>(lDctx->getExternalFunctionParameter(
          mapName)))) {
    ...
    jobject
    JdbcModule::getObject(const zorba::DynamicContext *aDynamincContext,
    ...
    void
    JdbcModule::deleteObject(const zorba::DynamicContext *aDynamincContext,
                             String aObjectUUID, String aMap) {
      InstanceMap *lInstanceMap = getCreateInstanceMap(aDynamincContext, aMap);
      if (lInstanceMap == NULL)
        throwMapError(aMap);
    ...
    void JdbcModule::initGlobals(const zorba::StaticContext *aStaticContext) {
      if (env != NULL)
        return;
      JDBC_MODULE_TRY;
      env = zorba::jvm::JavaVMSingleton::getInstance(
          aStaticContext)->getEnv();
      CHECK_EXCEPTION;
      jClass.init();
      jDriverManager.init();
      jConnection.init();
